...
import axios from "axios"
  export default {
    ...
    methods: {
        // 初始化 获取所有解析数据的函数
        clickHander() {
            axios.get('http://192.168.15.57:30000/')
            .then((data)=>{ // 正确的回调
       ...
       addHander(seqipDomain) {
            axios.post('http://192.168.15.57:30000/add',{'ipdomain': seqipDomain['ipDomain']})
            .then((data)=>{ // 正确的回调
       ...
       delHander(seqipDomain) {
            ...
            axios.post('http://192.168.15.57:30000/delete',{'ipdomain': seqipDomain['ipDomain']})
            .then((data)=>{ // 正确的回调
                this.timer = setTimeout(()=>{   //设置延迟执行刷新窗口
                    location.reload();
                },1000);
